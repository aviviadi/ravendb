...
Grow:
    ...
    public unsafe struct UnmanagedWriteBuffer2 : IUnmanagedWriteBuffer
    {
        private class Segment
        {
            /// <summary>
        ...
        public UnmanagedWriteBuffer2(int segmentSize)
        {

            var allocatedMemoryData = new AllocatedMemoryData
            {
                SizeInBytes = segmentSize,
                Address = (byte*)ElectricFencedMemory.Allocate(segmentSize)
            };
...
Grow:
        ...
        public void Dispose()
        {
            ...
            for (Segment next; head != null; head = next)
            {
                ...
                ElectricFencedMemory.Free(head.Address);
                // This is used to signal that Dispose has run to other copies
                head.Address = null;
